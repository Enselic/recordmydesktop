...
#Files page
        for i in range(2):
            self.labels[i]=Gtk.Label(label=prefLabelStrings[i])
            self.labels[i].set_justify(Gtk.Justification.LEFT)
...
#sound page
        ...
        self.jack_lsp_label=Gtk.Label(label=prefStrings['SelectPorts'])
        ...
        self.jack_lsp_listview.get_selection().set_mode(Gtk.SelectionMode.MULTIPLE)
...
#misc page
        ...
        self.mouseComboBox = Gtk.ComboBoxText()
        for i in range(4):
...
#append and show
        for i in range(4):
            self.notebook.append_page(self.labelbox[i],Gtk.Label(label=prefTabStrings[i]))
    ...
    def __runJackLSP__(self,button=None):
        ...
        if ports!=[]:
            ...
            self.jack_lsp_listview.get_selection().set_mode(Gtk.SelectionMode.NONE)
            self.values[23]=[]
        k=0
    ...
    def __register_shortcuts__(self):
        self.parent.advanced_button.add_accelerator("clicked",self.accel_group,
                                     ord('P'), Gdk.ModifierType.CONTROL_MASK,
                                     Gtk.AccelFlags.VISIBLE)
        self.parent.advanced_button.add_accelerator("clicked",self.accel_group,
                                     Gdk.keyval_from_name("Escape"),0,
                                     Gtk.AccelFlags.VISIBLE)
